...
const NavBar = ({toggle}) => {
  ...
  return (
    <nav
      className="flex justify-between items-center h-16 text-white relative shadow-sm font-mono bg-gray-600"
      role="navigation"
    >
      <NavLink
        to="/about"
      ...
      >
        <h2 className="intro" ref={(el) => (intro = el)}>
          <span className="text-blue-900">Ti</span>
          be<span className="text-red-600">tan</span>{' '}
          <span className="text-yellow-300">Ba</span><span style={{color:'#64AB70'}}>ke</span><span style={{color:"#E97910"}}>ry</span>
        </h2>
      </NavLink>
